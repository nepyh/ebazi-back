...
/**
 * 태스크 종료 시각이 지나면(그리고 완료 처리돼 있으면) 완료 확인 퀴즈를 자동 생성하고,
 * 직전 시도가 불합격이면 RETRY_DELAY_MINUTES(10)분 뒤 재시도를 자동 생성한다.
...
class TaskQuizTriggerJob(
    ...
    private fun findInitialTriggers(today: LocalDate, zonedNow: ZonedDateTime): List<DueJob> {
        ...
        return (PlanTaskTable innerJoin DailyPlanTable)
            .selectAll()
            .where { (DailyPlanTable.planDate eq today) and (PlanTaskTable.isCompleted eq true) }
            .mapNotNull { row ->
